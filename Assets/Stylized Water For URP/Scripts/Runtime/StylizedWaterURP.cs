...
namespace StylizedWaterForURP
{
    ...
    public class StylizedWaterURP : MonoBehaviour
    {
        ...
        private void ExecutePlanarReflections(ScriptableRenderContext context, Camera camera)
        {
            ...
            UpdateCamera(camera, _reflectionCamera);
            ...
            var oldPosition = camera.transform.position - new Vector3(0, pos.y * 2, 0);
            var newPosition = PlanarReflections.ReflectPosition(oldPosition);
            _reflectionCamera.transform.forward = Vector3.Scale(camera.transform.forward, new Vector3(1, -1, 1));
            ...
            if (camera.orthographicSize != 0) _reflectionCamera.projectionMatrix = projection;
            ...
            int zeroVectors = 0;
            if (_reflectionCamera.transform.rotation.x == 0)
                zeroVectors++;
            if (_reflectionCamera.transform.rotation.y == 0)
                zeroVectors++;
            if (_reflectionCamera.transform.rotation.z == 0)
                zeroVectors++;
            if(zeroVectors >1){
                FrustumError = true;
            }

            if (_reflectionCamera.WorldToViewportPoint(reflectionTarget.transform.position).z > 100000) FrustumError = true;
            if(clipPlane.w < -100000) FrustumError = true;

            if (!FrustumError) UniversalRenderPipeline.RenderSingleCamera(context, _reflectionCamera);
            data.Restore();
